...
//    readData();
  ...
  removeNews(NewsModel obj, int index)async{
    String URL = 'https://noobistani.000webhostapp.com/noobistani/deleteNews.php?id=${obj.id}&image_url=https://noobistani.000webhostapp.com/noobistani/${obj.imageUrl}';
...
//    readData();
  ...
  showSnackBar(context){
    Scaffold.of(context).showSnackBar(SnackBar(content: Text('News Removed'),));
  }
  @override
        ...
        builder: (BuildContext context, AsyncSnapshot<List> snapshot){
          if (snapshot.connectionState == ConnectionState.done) {
            if (snapshot.data == null) {
              return Center(child: Text('Why so empty here? :('));
            } else {
...
//    newsList.add(json.decode(response.body));
    for(var i = 0; i < test.length; i++){
      NewsModel newsPage = NewsModel(test[i]['id'], test[i]['date'], test[i]['description'], test[i]['image_url']);
